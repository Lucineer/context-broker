# CLAUDE.md — Context Broker

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Context Broker
- **Role**: Context Broker
- **URL**: https://context-broker.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/context-broker
- **Branch**: master
...
### Deploy
```bash
cd /tmp/context-broker && wrangler deploy
```

### Health Check
```bash
curl -s https://context-broker.casey-digennaro.workers.dev/health
curl -s https://context-broker.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
| /api/context | API endpoint |
| /api/goal | API endpoint |
| /api/goals | API endpoint |
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=context-broker
...
- **vessel.json capabilities**: goal-scoped-context, execution-thread
...
1. Keep the vessel small. ~150 lines is the current size.
